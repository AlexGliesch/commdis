...
int n_MaxPoints = 100; // for the test programm
...
int CircumCircle(double xp, double yp, double x1, double y1, double x2,
  ...
  if (abs(y1 - y2) < EPSILON && abs(y2 - y3) < EPSILON) return (false);
...
void randomize() { srand((time_t)time(NULL)); }
...
void outputtriangle(int&, XYZ* /* XYZ p[]*/, ITRIANGLE v[], int& ntri) {
  for (int i = 0; i < ntri;
       i++) { // replace cout by a compatible lineto to trace
    cout << i + 1 << ": " << v[i].p1 << " " << v[i].p2 << " " << v[i].p3
         << endl;
    // cout << (int)p[v[i].p1].x << " " << (int)p[v[i].p1].y << " "
    //      << (int)p[v[i].p2].x << " " << (int)p[v[i].p2].y << "\n";
    // cout << (int)p[v[i].p2].x << " " << (int)p[v[i].p2].y << " "
    //      << (int)p[v[i].p3].x << " " << (int)p[v[i].p3].y << "\n";
    // cout << (int)p[v[i].p3].x << " " << (int)p[v[i].p3].y << " "
    //      << (int)p[v[i].p1].x << " " << (int)p[v[i].p1].y << "\n";
  }
}

int main_test(int argc, char** argv) {
  ios_base::sync_with_stdio(false);
  if (argc >= 2) n_MaxPoints = atoi(argv[1]);
  ITRIANGLE* v = NULL;
  int max = 10;
  XYZ* p = new XYZ[max];
  XYZ* p_Temp = NULL;
